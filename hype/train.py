...
def train(
    ...
    for epoch in range(opt.epoch_start, opt.epochs):
        ...
        for i_batch, (inputs, targets) in enumerate(loader_iter):
            ...
            counts.copy_(th.from_numpy(
                np.bincount(
                    inputs.view(-1).numpy(), minlength=model.nobjects
                )[:, np.newaxis]
            ))
            counts.clamp_(min=1)
            ...
            loss.backward()
            optimizer.step(lr=lr, counts=counts.div_(inputs.size(0)))
            epoch_loss[i_batch] = loss.item()
        if hasattr(data, 'avg_queue_size'):
            qsize = data.avg_queue_size()
            misses = data.queue_misses()
            log.info(f'Average queue size for epoch was {qsize}, num_misses={misses}')
