...
cdef class BatchedDataset:
    ...
    cdef public int neg_multiplier
    ...
    def __cinit__(self, idx, objects, weights, nnegs, batch_size, num_workers, burnin = False):
        '''
    ...
    def _mk_weights(self, npc.ndarray[npc.long_t, ndim=2] idx, npc.ndarray[npc.double_t, ndim=1] weights):
        ...
        if self.burnin:
            ...
            # https://aaai.org/ocs/index.php/AAAI/AAAI17/paper/view/14956
    ...
    cpdef _worker(self, i):
        cdef long [:,:] memview
        while self.current + self.batch_size <= self.idx.shape[0]:
            current = self.current
            self.current += self.batch_size
            ix = torch.LongTensor(self.batch_size, self.nnegs + 2)
            memview = ix.numpy()
            with nogil:
                self._getbatch(current, memview)
            self.queue.put((ix, torch.zeros(self.batch_size).long()))
        self.queue.put(i)
    ...
    def __len__(self):
        return int(np.floor(self.idx.shape[0] / self.batch_size))
    ...
    cdef public void _getbatch(self, int i, long[:,:] ix) nogil:
        '''
        ...
        for j in range(self.batch_size):
            ...
            while ntries < self.max_tries and negs.size() < self.nnegs:
                if self.burnin:
                ...
                if self._weights[t].find(n) == self._weights[t].end() or (self._weights[t][n] < weight_th):
            ...
            while ixptr < self.nnegs + 2:
                ix[j, ixptr] = ix[j, 2 + <long>(<double>rand_r(&seed)/RAND_MAX*(ixptr-2))]
                ixptr = ixptr + 1

    def nnegatives(self):
        if self.burnin:
            return self.neg_multiplier * self.nnegs
        else:
            return self.nnegs
